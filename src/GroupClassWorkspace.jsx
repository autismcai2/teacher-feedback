      ...
      <main className="groupMain">
        ...
        <section className="groupCard classSettings">
          ...
          <div className="classInfoGrid">
            <label className="groupField"><span>班级</span><select value={classInfo.classId} onChange={(event) => selectClass(event.target.value)}>{classProfiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.title} · {profile.grade}{profile.subject}</option>)}</select></label>
            <Field label="年级" readOnly value={classInfo.grade} onChange={() => {}} />
            <Field label="科目" readOnly value={classInfo.subject} onChange={() => {}} />
            <Field label="上课日期" type="date" value={classInfo.date} onChange={(value) => updateClassInfo("date", value)} />
            <ClassTimeField value={classInfo.time} mode={classInfo.timeMode} onChange={(time, timeMode) => setClassInfo((previous) => ({ ...previous, time, timeMode }))} />
            ...
            <table className="studentTable">
              ...
              <tbody>{filteredStudents.map((student) => {
                ...
                <tr key={student.id}>
                  ...
                  <td><button className={`scoreCell ${student.score !== "" ? "filled" : ""}`} type="button" onClick={() => openRecorder(index)}>{student.score === "" ? "+ 录分" : student.score === "缺考" ? "缺考" : `${student.score} / 30`}</button></td>
