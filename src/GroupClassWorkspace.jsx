            ...
            <table className="studentTable">
              ...
              <tbody>{filteredStudents.map((student) => {
                ...
                <tr key={student.id}>
                  ...
                  <td><div className="studentName"><span>{student.name.slice(-1)}</span><input value={student.name} onChange={(event) => updateStudent(student.id, { name: event.target.value })} /></div></td>
        ...
        <div className="scoreOverlay" role="dialog" aria-modal="true" aria-label="连续录入入门测成绩">
          ...
          <section className="scoreRecorder">
            ...
            {scoreMode === "pick" ? <>
              ...
              <div className="absentRow"><button type="button" onClick={markAbsent}>缺考</button><p className="autoHint">选择后将在 0.65 秒后自动进入下一位</p></div>
