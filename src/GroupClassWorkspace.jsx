...
export default function GroupClassWorkspace({ onBack }) {
  ...
  async function generateDraft() {
    ...
    try {
      ...
      setStudents((previous) => previous.map((student, index) => ({ ...student, quickNote: Array.from(comments.get(student.name) || student.quickNote || commentSeeds[index % commentSeeds.length]).slice(0, 12).join("") })));
      ...
      <main className="groupMain">
        ...
        <section className="studentWorkspace">
          ...
          <div className="studentCardList">{filteredStudents.map((student) => { const index = students.findIndex((item) => item.id === student.id); return <article className={`studentRecordCard ${editingStudent === student.id ? "editing" : ""}`} key={student.id}>
            ...
            <div className="studentCardBottom"><input className="quickInput" maxLength={12} value={student.quickNote} onFocus={() => setEditingStudent(student.id)} onBlur={() => setEditingStudent("")} onChange={(event) => updateStudent(student.id, { quickNote: event.target.value })} placeholder="课堂表现（8—12字，可留空）" /><button className={`scoreCell ${student.score !== "" ? "filled" : ""}`} type="button" onClick={() => openRecorder(index)}>{student.score === "" ? "+ 录分" : student.score === "缺考" ? "缺考" : `${student.score} / 30`}</button></div>
          </article> })}</div>
